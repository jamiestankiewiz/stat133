...
# Use car model names as labels of bubbles;
...
optionlist <- list(title="Fuel Consumption of Cars", 
                   hAxis="{title:'disp'}",
                   vAxis="{title:'mpg'}")

bub2 <- gvisBubbleChart(mtcars, idvar="model", xvar="disp", yvar="mpg",
                       colorvar="gear", sizevar="hp", options=optionlist)

# Now plot your bubble chart, the chart will show up in a new tab in your web browser.
plot(bub2)
...
valid_ind <- apply(WorldDat, 1, row.na)
WorldDat <- WorldDat[valid_ind, ]
...
Motion <- gvisMotionChart(WorldDat, idvar="country", timevar="year", colorvar="region")
